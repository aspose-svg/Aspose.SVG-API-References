...
description: "Méthode SVGBuilderExtensions AddFeDisplacementMap. Ajoute une configuration d'élément feDisplacementMap au constructeur. Cet élément décale une image selon une carte vectorielle spécifiée"
type: docs
weight: 190
url: /fr/net/aspose.svg.builder/svgbuilderextensions/addfedisplacementmap/
---
## AddFeDisplacementMap<TBuilder>(*this TBuilder, Action&lt;SVGFEDisplacementMapElementBuilder&gt;*) {#addfedisplacementmap}

Ajoute une configuration d'élément 'feDisplacementMap' au constructeur. Cet élément décale une image selon une carte vectorielle spécifiée.
...
| configurer | L'action de configuration pour l'élément 'feDisplacementMap'. |
...
Ajoute un élément 'feDisplacementMap' au constructeur SVG, créant un effet qui déforme une image en fonction des données de couleur d'une seconde source.
...
| constructeur | L'instance du constructeur SVG à laquelle l'élément 'feDisplacementMap' sera ajouté. |
| scale | Le facteur d'échelle déterminant la quantité de déplacement. Paramètre optionnel. |
| xChannelSelector | Le canal de l'image in2 à utiliser pour le déplacement le long de l'axe x. Paramètre optionnel. |
| yChannelSelector | Le canal de l'image in2 à utiliser pour le déplacement le long de l'axe y. Paramètre optionnel. |
| in | L'image d'entrée à déplacer. Peut être une chaîne ou un FilterInput. Paramètre optionnel. |
| in2 | L'image fournissant les données de déplacement. Peut être une chaîne ou un FilterInput. Paramètre optionnel. |
...
| étendre | Une action optionnelle pour configurer davantage le SVGFEDisplacementMapElementBuilder. |
