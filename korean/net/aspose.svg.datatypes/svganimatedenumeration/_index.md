---
title: Class SVGAnimatedEnumeration
second_title: .NET API 참조용 Aspose.SVG
description: Aspose.Svg.DataTypes.SVGAnimatedEnumeration 수업. 값이 특정 열거의 상수여야 하고 애니메이션할 수 있는 속성에 사용됩니다.
type: docs
weight: 110
url: /ko/net/aspose.svg.datatypes/svganimatedenumeration/
---
## SVGAnimatedEnumeration class

값이 특정 열거의 상수여야 하고 애니메이션할 수 있는 속성에 사용됩니다.

```csharp
public class SVGAnimatedEnumeration : SVGAnimatedValue<ushort>
```

## 속성

| 이름 | 설명 |
| --- | --- |
| virtual [AnimVal](../../aspose.svg.datatypes/svganimatedvalue-1/animval/) { get; } |  |
| [BaseVal](../../aspose.svg.datatypes/svganimatedvalue-1/baseval/) { get; set; } |  |

## 행동 양식

| 이름 | 설명 |
| --- | --- |
| [Dispose](../../aspose.svg.datatypes/svgvaluetype/dispose/)() | 관리되지 않는 리소스와 선택적으로 관리되는 리소스를 해제합니다. |
| virtual [GetPlatformType](../../aspose.svg.dom/domobject/getplatformtype/)() | 이 메서드는 ECMAScript 개체를 검색하는 데 사용됩니다.Type . |
| override [ToString](../../aspose.svg.datatypes/svganimatedenumeration/tostring/)() | 반환String 이 instance. 를 나타냅니다. |

### 또한보십시오

* class [SVGAnimatedValue&lt;T&gt;](../svganimatedvalue-1/)
* 네임스페이스 [Aspose.Svg.DataTypes](../../aspose.svg.datatypes/)
* 집회 [Aspose.SVG](../../)


