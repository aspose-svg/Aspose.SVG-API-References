...
description: Aspose.Svg.Dom.CDATASection 수업. CDATA 섹션은 그렇지 않으면 마크업으로 간주되는 문자를 포함하는 텍스트 블록을 이스케이프하는 데 사용됩니다.
type: docs
weight: 440
url: /ko/net/aspose.svg.dom/cdatasection/
---
## CDATASection class
...
| virtual [Data](../../aspose.svg.dom/characterdata/data/) { get; set; } | 이 인터페이스를 구현하는 노드의 문자 데이터입니다. |
| [FirstChild](../../aspose.svg.dom/node/firstchild/) { get; } | 이 노드의 첫 번째 자식입니다. 해당 노드가 없으면 null을 반환합니다. |
| [IsElementContentWhitespace](../../aspose.svg.dom/text/iselementcontentwhitespace/) { get; } | 이 텍스트 노드에 종종 "무시할 수 있는 공백"이라고 욕설로 불리는 요소 콘텐츠 공백이 포함되어 있는지 여부를 반환합니다. |
| [LastChild](../../aspose.svg.dom/node/lastchild/) { get; } | 이 노드의 마지막 자식입니다. 해당 노드가 없으면 null을 반환합니다. |
| [Length](../../aspose.svg.dom/characterdata/length/) { get; } | data와 아래의 substringData 메소드를 통해 사용 가능한 16비트 단위의 개수입니다. 이것은 값이 0일 수 있습니다. 즉, CharacterData 노드가 비어 있을 수 있습니다. |
...
| override [NodeName](../../aspose.svg.dom/cdatasection/nodename/) { get; } | 유형에 따라 이 노드의 이름입니다. |
| override [NodeType](../../aspose.svg.dom/cdatasection/nodetype/) { get; } | 기본 개체의 유형을 나타내는 코드입니다. |
| override [NodeValue](../../aspose.svg.dom/text/nodevalue/) { get; set; } | 유형에 따라 이 노드의 값입니다. |
...
| override [TextContent](../../aspose.svg.dom/text/textcontent/) { get; set; } | 이 속성은 이 노드와 그 자손의 텍스트 내용을 반환합니다. null로 정의된 경우 설정해도 아무런 효과가 없습니다. 설정 시 이 노드가 가질 수 있는 모든 가능한 자식이 제거되고 새 문자열이 비어 있거나 null이 아닌 경우 이 속성이 설정된 문자열을 포함하는 단일 텍스트 노드로 대체됩니다. |
| [WholeText](../../aspose.svg.dom/text/wholetext/) { get; } | 이 노드에 논리적으로 인접한 텍스트 노드의 모든 텍스트를 문서 순서로 연결하여 반환합니다. |
...
| virtual [AppendData](../../aspose.svg.dom/characterdata/appenddata/)(string) | 노드의 문자 데이터 끝에 문자열을 추가합니다. |
...
| virtual [DeleteData](../../aspose.svg.dom/characterdata/deletedata/)(int, int) | 노드에서 16비트 단위 범위를 제거합니다. |
...
| virtual [InsertData](../../aspose.svg.dom/characterdata/insertdata/)(int, string) | 지정된 16비트 단위 오프셋에 문자열을 삽입합니다. |
...
| virtual [ReplaceData](../../aspose.svg.dom/characterdata/replacedata/)(int, int, string) | 지정된 16비트 단위 오프셋에서 시작하는 문자를 지정된 문자열로 바꿉니다. |
| [ReplaceWholeText](../../aspose.svg.dom/text/replacewholetext/)(string) | 현재 노드의 텍스트와 논리적으로 인접한 모든 텍스트 노드를 지정된 텍스트로 바꿉니다. 대체 텍스트의 수신자가 아닌 한 현재 노드를 포함하여 논리적으로 인접한 모든 텍스트 노드가 제거됩니다. |
| [SplitText](../../aspose.svg.dom/text/splittext/)(int) | 이 노드를 지정된 오프셋에서 두 개의 노드로 나누고 둘 다 트리에서 형제로 유지합니다. |
| virtual [SubstringData](../../aspose.svg.dom/characterdata/substringdata/)(int, int) | 노드에서 데이터 범위를 추출합니다. |
| override [ToString](../../aspose.svg.dom/characterdata/tostring/)() | 반환String 이 instance. 를 나타냅니다. |
