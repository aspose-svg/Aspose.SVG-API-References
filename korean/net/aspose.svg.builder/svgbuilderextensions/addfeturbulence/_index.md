...
description: "SVGBuilderExtensions AddFeTurbulence 메서드. 빌더에 feTurbulence 요소 구성을 추가합니다. 이 요소는 구름이나 대리석과 같은 텍스처를 만드는 데 유용한 퍼린 노이즈를 사용하여 이미지를 생성합니다."
type: docs
weight: 290
url: /ko/net/aspose.svg.builder/svgbuilderextensions/addfeturbulence/
---
## AddFeTurbulence<TBuilder>(*this TBuilder, Action&lt;SVGFETurbulenceElementBuilder&gt;*) {#addfeturbulence_1}

빌더에 'feTurbulence' 요소 구성을 추가합니다. 이 요소는 퍼린 노이즈를 사용해 이미지를 생성하며, 구름이나 대리석과 같은 텍스처를 만드는 데 유용합니다.
...
SVG 빌더에 'feTurbulence' 요소를 추가하여 퍼린 노이즈를 사용해 구름이나 대리석과 같은 난류 효과를 만듭니다.
...
| 빌더 | 'feTurbulence' 요소가 추가될 SVG 빌더 인스턴스. |
| baseFrequency | 터뷸런스의 기본 주파수. double 또는 두 개의 double을 가진 ValueTuple일 수 있습니다. 선택 매개변수. |
| numOctaves | 터뷸런스의 옥타브 수. 선택 매개변수. |
| seed | 난수 생성기의 시드 번호. 선택 매개변수. |
| stitchTiles | 타일이 함께 스티치되는지 여부를 나타냅니다. 선택 매개변수. |
| type | 터뷸런스 유형 (프랙탈 노이즈 또는 터뷸런스). 선택 매개변수. |
| in | 터뷸런스 효과가 적용될 입력 이미지. 문자열 또는 FilterInput일 수 있습니다. 선택 매개변수. |
...
| extend | SVGFETurbulenceElementBuilder를 추가로 구성하기 위한 선택적 작업입니다. |
