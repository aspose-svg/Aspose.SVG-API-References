---
title: "SVGBuilderExtensions.OnAbort"
second_title: "Aspose.SVG for .NET API 참조"
description: "SVGBuilderExtensions OnAbort 메서드. SVG 문서 로드가 중단될 때 실행될 스크립트를 정의하는 onabort 이벤트 속성을 설정합니다."
type: docs
weight: 1190
url: /ko/net/aspose.svg.builder/svgbuilderextensions/onabort/
---
## SVGBuilderExtensions.OnAbort<TBuilder> method

'onabort' 이벤트 속성을 설정하고, SVG 문서 로드가 중단될 때 실행될 스크립트를 정의합니다.

```csharp
public static TBuilder OnAbort<TBuilder>(this TBuilder builder, string value)
    where TBuilder : ISVGElementBuilder, IDocumentEventAttributeSetter
```

| 매개변수 | 설명 |
| --- | --- |
| TBuilder | SVG 요소 빌더의 유형입니다. |
| 빌더 | SVG 요소 빌더. |
| 값 | 문서 로드가 중단될 때 실행할 JavaScript 함수 또는 스크립트. |

### 반환 값

체이닝을 위한 빌더 인스턴스입니다.

### 또 보기

* interface [ISVGElementBuilder](../../isvgelementbuilder/)
* interface [IDocumentEventAttributeSetter](../../idocumenteventattributesetter/)
* class [SVGBuilderExtensions](../)
* namespace [Aspose.Svg.Builder](../../../aspose.svg.builder/)
* assembly [Aspose.SVG](../../../)
