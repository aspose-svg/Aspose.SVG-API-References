...
description: "SVGBuilderExtensions AddFeDiffuseLighting 메서드. 빌더에 feDiffuseLighting 요소 구성을 추가합니다. 이 요소는 이미지에 조명 효과를 제공합니다."
type: docs
weight: 180
url: /ko/net/aspose.svg.builder/svgbuilderextensions/addfediffuselighting/
---
## AddFeDiffuseLighting<TBuilder>(*this TBuilder, Action&lt;SVGFEDiffuseLightingElementBuilder&gt;*) {#addfediffuselighting}

빌더에 'feDiffuseLighting' 요소 구성을 추가합니다. 이 요소는 이미지에 조명 효과를 제공합니다.
...
| 빌더 | 빌더 인스턴스입니다. |
| 구성 | 'feDiffuseLighting' 요소에 대한 구성 작업. |
...
SVG 빌더에 'feDiffuseLighting' 요소를 추가하고, 지정된 광원을 사용하여 확산 조명 효과를 적용합니다.
...
| 빌더 | 'feDiffuseLighting' 요소가 추가될 SVG 빌더 인스턴스. |
| lightSource | 디퓨즈 조명 효과를 위한 광원 구성을 수행하는 작업. |
| lightingColor | 광원의 색상입니다. 선택적 매개변수. |
| surfaceScale | 조명 효과를 위한 표면 스케일 팩터입니다. 선택적 매개변수. |
| diffuseConstant | 조명 효과를 결정하는 데 사용되는 상수. 선택적 매개변수. |
| in | 디퓨즈 조명 효과에 대한 입력. 문자열이나 FilterInput일 수 있습니다. 선택적 매개변수. |
...
| extend | SVGFEDiffuseLightingElementBuilder를 추가로 구성하기 위한 선택적 작업. |
