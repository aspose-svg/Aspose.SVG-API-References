...
description: "SVGBuilderExtensions AddFeConvolveMatrix 메서드. builder에 feConvolveMatrix 요소 구성을 추가합니다. 이 요소는 매트릭스 컨볼루션 필터 효과를 적용합니다."
type: docs
weight: 170
url: /ko/net/aspose.svg.builder/svgbuilderextensions/addfeconvolvematrix/
---
## AddFeConvolveMatrix<TBuilder>(*this TBuilder, Action&lt;SVGFEConvolveMatrixElementBuilder&gt;*) {#addfeconvolvematrix_1}

빌더에 'feConvolveMatrix' 요소 구성을 추가합니다. 이 요소는 행렬 컨볼루션 필터 효과를 적용합니다.
...
| 빌더 | 빌더 인스턴스입니다. |
| 구성 | 'feConvolveMatrix' 요소에 대한 구성 작업입니다. |
...
SVG 빌더에 'feConvolveMatrix' 요소를 추가하여 행렬 컨볼루션 필터 효과를 적용합니다.
...
| 빌더 | 'feConvolveMatrix' 요소가 추가될 SVG builder 인스턴스입니다. |
| kernelMatrix | 컨볼루션에 사용되는 값들의 매트릭스입니다. 선택적 매개변수. |
| divisor | 컨볼루션에 사용되는 나눗값입니다. 선택적 매개변수. |
| bias | 컨볼루션 결과에 추가되는 바이어스입니다. 선택적 매개변수. |
| targetX | 커널 매트릭스에서 대상 픽셀의 x 좌표입니다. 선택적 매개변수. |
| targetY | 커널 매트릭스에서 대상 픽셀의 y 좌표입니다. 선택적 매개변수. |
| edgeMode | 컨볼루션에서 가장자리 픽셀을 처리하는 방식을 정의합니다. 선택적 매개변수. |
| preserveAlpha | 알파 채널을 보존할지 여부를 나타냅니다. 선택적 매개변수. |
| order | 커널 매트릭스의 차수입니다. int 또는 두 개의 int로 구성된 ValueTuple일 수 있습니다. 선택적 매개변수. |
| in | 컨볼루션 효과에 대한 입력입니다. 문자열 또는 FilterInput일 수 있습니다. 선택적 매개변수. |
...
| extend | SVGFEConvolveMatrixElementBuilder를 추가로 구성하기 위한 선택적 작업입니다. |
