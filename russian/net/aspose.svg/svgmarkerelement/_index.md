...
description: Aspose.Svg.SVGMarkerElement сорт. Интерфейс SVGMarkerElement соответствует элементу маркер.
type: docs
weight: 3340
...
| [OrientAngle](../../aspose.svg/svgmarkerelement/orientangle/) { get; } | Соответствует атрибуту «ориентация» данного элемента «маркер». Если markerUnits равно SVG_MARKER_ORIENT_ANGLE, значение угла для атрибута 'ориентация'; в противном случае он будет равен нулю. |
...
| [SetOrientToAuto](../../aspose.svg/svgmarkerelement/setorienttoauto/)() | Устанавливает значение атрибута 'ориентация' на 'авто'. |
...
| const [SVG_MARKERUNITS_UNKNOWN](../../aspose.svg/svgmarkerelement/svg_markerunits_unknown/) | Тип единицы маркера не является одним из предопределенных типов. Недопустимо пытаться определить новое значение этого типа или пытаться переключить существующее значение на этот тип. |
...
| const [SVG_MARKER_ORIENT_UNKNOWN](../../aspose.svg/svgmarkerelement/svg_marker_orient_unknown/) | Ориентация маркера не является одним из предопределенных типов. Недопустимо пытаться определить новое значение этого типа или пытаться переключить существующее значение на этот тип. |
