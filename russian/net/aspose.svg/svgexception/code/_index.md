---
title: Code
second_title: Справочник по Aspose.SVG для .NET API
description: Код определяющий причину по которой запрошенная операция не может быть выполнена. Значение этого члена будет одной из констант в кодовой группе SVGException.
type: docs
weight: 20
url: /ru/net/aspose.svg/svgexception/code/
---
## SVGException.Code property

Код, определяющий причину, по которой запрошенная операция не может быть выполнена. Значение этого члена будет одной из констант в кодовой группе SVGException.

```csharp
public ushort Code { get; }
```

### Стоимость имущества

Код ошибки.

### Смотрите также

* class [SVGException](../../svgexception)
* пространство имен [Aspose.Svg](../../svgexception)
* сборка [Aspose.SVG](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.SVG.dll -->
