...
description: "Aspose.Svg.SVGTextContentElement class. SVGTextContentElement наследуется различными интерфейсами, связанными с текстом, такими как SVGTextElement, SVGTSpanElement, SVGTRefElement, SVGAltGlyphElement и SVGTextPathElement. Для методов этого интерфейса, которые ссылаются на индекс символа или количество символов, такие ссылки следует интерпретировать как индекс кода UTF-16 или количество кодов UTF-16 соответственно. Это сделано для согласованности с DOM Level 2 Core, где методы интерфейса CharacterData используют коды UTF-16 в качестве индексов и счётчиков внутри данных символов. Таким образом, например, если содержимое текстового элемента представляет один символ за пределами BMP, например U10000, вызов getNumberOfChars для этого элемента вернёт 2, поскольку для представления этого одного символа используется пара суррогатов из двух кодов UTF-16."
type: docs
weight: 5590
...
Интерфейс SVGTextContentElement наследуется различными интерфейсами, связанными с текстом, такими как SVGTextElement, SVGTSpanElement, SVGTRefElement, SVGAltGlyphElement и SVGTextPathElement. Для методов этого интерфейса, которые ссылаются на индекс символа или количество символов, эти ссылки следует интерпретировать соответственно как индекс к коду единицы UTF-16 или количество единиц кода UTF-16. Это делается для согласованности с DOM Level 2 Core, где методы интерфейса CharacterData используют единицы кода UTF-16 в качестве индексов и счётчиков внутри символьных данных. Таким образом, например, если текстовое содержимое элемента ‘text’ представляет собой один символ за пределами BMP, например U+10000, то вызов getNumberOfChars для этого элемента вернёт 2, поскольку для представления этого одного символа используется две единицы кода UTF-16 (пара суррогатов).
...
| [LengthAdjust](../../aspose.svg/svgtextcontentelement/lengthadjust/) { get; } | Соответствует атрибуту ‘lengthAdjust’ данного элемента. Значение должно быть одной из констант регулировки длины, определённых в этом интерфейсе. |
...
| [GetComputedTextLength](../../aspose.svg/svgtextcontentelement/getcomputedtextlength/)() | Общая сумма всех значений продвижения, полученных при рендеринге всех символов внутри этого элемента, включая значение продвижения глифов (горизонтальное или вертикальное), влияние свойств ‘kerning’, ‘letter-spacing’ и ‘word-spacing’ и корректировки, вызванные атрибутами ‘dx’ и ‘dy’ в элементах ‘tspan’. Для сред, не выполняющих рендеринг, пользовательский агент должен делать разумные предположения о метриках глифов. |
...
| const [LENGTHADJUST_UNKNOWN](../../aspose.svg/svgtextcontentelement/lengthadjust_unknown/) | Перечисление было установлено в значение, которое не является одним из предопределённых типов. Недопустимо пытаться определить новое значение этого типа или пытаться переключить существующее значение на этот тип. |
