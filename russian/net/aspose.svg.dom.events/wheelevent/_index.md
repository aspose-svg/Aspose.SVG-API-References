...
| [DeltaMode](../../aspose.svg.dom.events/wheelevent/deltamode) { get; } | Атрибут deltaMode содержит указание единиц измерения для дельта-значений. Значение по умолчанию — DOM_DELTA_PIXEL (в пикселях). |
| [DeltaX](../../aspose.svg.dom.events/wheelevent/deltax) { get; } | В пользовательских агентах, где действием по умолчанию для события колеса является прокрутка, значение ДОЛЖНО быть измерением по оси x (в пикселях, строках или страницах) для прокрутки. случай, когда мероприятие не отменяется. В противном случае это зависящее от реализации измерение (в пикселях, строках или страницах) движения колесика вокруг оси X. |
| [DeltaY](../../aspose.svg.dom.events/wheelevent/deltay) { get; } | В пользовательских агентах, где действием по умолчанию для события колеса является прокрутка, значение ДОЛЖНО быть измерением по оси Y (в пикселях, строках или страницах) для прокрутки. случай, когда мероприятие не отменяется. В противном случае это зависящее от реализации измерение (в пикселях, строках или страницах) движения колесика вокруг оси Y. |
| [DeltaZ](../../aspose.svg.dom.events/wheelevent/deltaz) { get; } | В пользовательских агентах, где действие колеса по умолчанию — прокрутка, значение ДОЛЖНО быть измерением вдоль оси z (в пикселях, строках или страницах) для прокрутки. случай, когда мероприятие не отменяется. В противном случае это зависящее от реализации измерение (в пикселях, строках или страницах) движения колесика вокруг оси Z. |
...
| const [DOM_DELTA_PAGE](../../aspose.svg.dom.events/wheelevent/dom_delta_page) | Единицами измерения дельты ДОЛЖНЫ быть страницы, определенные либо как отдельный экран, либо как разграниченная страница. |
