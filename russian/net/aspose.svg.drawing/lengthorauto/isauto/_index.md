---
title: IsAuto
second_title: Справочник по Aspose.SVG для .NET API
description: Получает значение указывающее находится ли данный экземпляр в состоянии auto.
type: docs
weight: 20
url: /ru/net/aspose.svg.drawing/lengthorauto/isauto/
---
## LengthOrAuto.IsAuto property

Получает значение, указывающее, находится ли данный экземпляр в состоянии 'auto'.

```csharp
public bool IsAuto { get; }
```

### Стоимость имущества

` true` если этот экземпляр автоматический; в противном случае` false` .

### Смотрите также

* class [LengthOrAuto](../../lengthorauto)
* пространство имен [Aspose.Svg.Drawing](../../lengthorauto)
* сборка [Aspose.SVG](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.SVG.dll -->
