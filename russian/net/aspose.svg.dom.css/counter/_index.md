---
title: Counter
second_title: Справочник по Aspose.SVG для .NET API
description: Интерфейс счетчика используется для представления любого счетчика или значения функции счетчика. Этот интерфейс отражает значения в базовом свойстве стиля.
type: docs
weight: 520
url: /ru/net/aspose.svg.dom.css/counter/
---
## Counter class

Интерфейс счетчика используется для представления любого счетчика или значения функции счетчика. Этот интерфейс отражает значения в базовом свойстве стиля.

```csharp
public class Counter : DOMObject
```

## Характеристики

| Имя | Описание |
| --- | --- |
| [Identifier](../../aspose.svg.dom.css/counter/identifier) { get; } | Этот атрибут используется для идентификатора счетчика. |
| [ListStyle](../../aspose.svg.dom.css/counter/liststyle) { get; } | Этот атрибут используется для стиля списка. |
| [Separator](../../aspose.svg.dom.css/counter/separator) { get; } | Этот атрибут используется для разделителя вложенных счетчиков. |

## Методы

| Имя | Описание |
| --- | --- |
| virtual [GetPlatformType](../../aspose.svg.dom/domobject/getplatformtype)() | Этот метод используется для получения объекта ECMAScriptType. |

### Смотрите также

* class [DOMObject](../../aspose.svg.dom/domobject)
* пространство имен [Aspose.Svg.Dom.Css](../../aspose.svg.dom.css)
* сборка [Aspose.SVG](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.SVG.dll -->
