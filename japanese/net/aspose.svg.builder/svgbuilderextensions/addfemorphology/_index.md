...
description: "SVGBuilderExtensions AddFeMorphology メソッド。builder に feMorphology 要素の構成を追加します。この要素は入力画像に対して膨張や侵食などの形態学的操作を適用するために使用されます。"
type: docs
weight: 250
url: /ja/net/aspose.svg.builder/svgbuilderextensions/addfemorphology/
---
## AddFeMorphology<TBuilder>(*this TBuilder, Action&lt;SVGFEMorphologyElementBuilder&gt;*) {#addfemorphology}

ビルダーに 'feMorphology' 要素の構成を追加します。この要素は入力画像に対して膨張や侵食などの形態学的操作を適用するために使用されます。
...
SVG ビルダーに 'feMorphology' 要素を追加し、入力画像に形態学的操作を適用します。
...
| ビルダー | 「feMorphology」要素が追加される SVG ビルダーインスタンス。 |
| operator | 適用される形態学的オペレーター。オプション パラメータ。 |
| radius | 形態学的操作の半径。double または 2 つの double の ValueTuple として指定できます。オプション パラメータ。 |
| in | 形態学的操作が適用される入力画像です。文字列または FilterInput にすることができます。オプション パラメーターです。 |
...
| extend | SVGFEMorphologyElementBuilder をさらに構成するためのオプションのアクションです。 |
