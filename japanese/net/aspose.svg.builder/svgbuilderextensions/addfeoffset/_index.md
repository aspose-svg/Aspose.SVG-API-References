...
description: "SVGBuilderExtensions AddFeOffset メソッド。builder に feOffset 要素の構成を追加します。この要素は指定されたベクトルで入力画像をオフセットします。"
type: docs
weight: 260
url: /ja/net/aspose.svg.builder/svgbuilderextensions/addfeoffset/
---
## AddFeOffset<TBuilder>(*this TBuilder, Action&lt;SVGFEOffsetElementBuilder&gt;*) {#addfeoffset}

ビルダーに 'feOffset' 要素の構成を追加します。この要素は指定されたベクトルで入力画像をオフセットします。
...
SVG ビルダーに 'feOffset' 要素を追加し、指定されたベクトルで入力画像をシフトしてオフセット効果を作成します。
...
| ビルダー | 'feOffset' 要素が追加される SVG ビルダーインスタンスです。 |
| dx | 水平オフセット距離です。オプションパラメータ。 |
| dy | 垂直オフセット距離です。オプションパラメータ。 |
| in | オフセットが適用される入力画像です。文字列または FilterInput にできます。オプションパラメータ。 |
...
| extend | SVGFEOffsetElementBuilder をさらに構成するためのオプションアクションです。 |
