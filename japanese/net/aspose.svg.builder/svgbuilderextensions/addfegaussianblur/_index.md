...
description: "SVGBuilderExtensions AddFeGaussianBlur メソッド。feGaussianBlur 要素の構成をビルダーに追加します。この要素は入力画像にガウスぼかしを適用します。"
type: docs
weight: 220
url: /ja/net/aspose.svg.builder/svgbuilderextensions/addfegaussianblur/
---
## AddFeGaussianBlur<TBuilder>(*this TBuilder, Action&lt;SVGFEGaussianBlurElementBuilder&gt;*) {#addfegaussianblur_1}

ビルダーに 'feGaussianBlur' 要素の構成を追加します。この要素は入力画像にガウスぼかしを適用します。
...
| ビルダー | ビルダー インスタンス。 |
| 構成 | 'feGaussianBlur' 要素の構成アクションです。 |
...
SVG ビルダーに 'feGaussianBlur' 要素を追加し、入力画像にガウスぼかし効果を適用します。
...
| ビルダー | 'feGaussianBlur' 要素が追加される SVG ビルダーインスタンスです。 |
| stdDeviation | ぼかし操作の標準偏差です。double または 2 つの double の ValueTuple を指定できます。省略可能なパラメータです。 |
| in | ガウスぼかしが適用される入力画像です。文字列または FilterInput を指定できます。省略可能なパラメータです。 |
...
| extend | SVGFEGaussianBlurElementBuilder をさらに構成するためのオプションのアクションです。 |
