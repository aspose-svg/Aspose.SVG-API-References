...
description: Aspose.Svg.Dom.Comment クラス. CharacterData から継承しコメントの内容を表しますつまり先頭の  .
type: docs
weight: 460
...
| override [NodeName](../../aspose.svg.dom/comment/nodename/) { get; } | タイプに応じたこのノードの名前. |
| override [NodeType](../../aspose.svg.dom/comment/nodetype/) { get; } | 基礎となるオブジェクトのタイプを表すコード. |
| override [NodeValue](../../aspose.svg.dom/comment/nodevalue/) { get; set; } | タイプに応じたこのノードの値. |
...
| override [TextContent](../../aspose.svg.dom/comment/textcontent/) { get; set; } | この属性は、このノードとその子孫のテキスト コンテンツを返します。 null として定義されている場合、設定しても効果はありません。設定時に、このノードが持つ可能性のある子はすべて削除され、新しい文字列が空または null でない場合は、この属性が設定されている文字列を含む単一の Text ノードに置き換えられます。 |
