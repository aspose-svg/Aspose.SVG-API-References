...
description: "Aspose.Svg.Dom.Comment クラス。CharacterData から継承し、コメントの内容、すなわち開始タグと終了タグの間のすべての文字を表します"
type: docs
weight: 2460
url: /ja/net/aspose.svg.dom/comment/
---
## Comment class

CharacterData から継承し、コメントの内容、すなわち開始 '' と終了 '' の間のすべての文字を表します。
...
| override [NodeName](../../aspose.svg.dom/comment/nodename/) { get; } | このノードの名前（タイプに応じて）。 |
| override [NodeType](../../aspose.svg.dom/comment/nodetype/) { get; } | 基になるオブジェクトのタイプを表すコード。 |
| override [NodeValue](../../aspose.svg.dom/comment/nodevalue/) { get; set; } | このノードの値（タイプに応じて）。 |
...
| override [TextContent](../../aspose.svg.dom/comment/textcontent/) { get; set; } | この属性はこのノードとその子孫のテキスト内容を返します。null に設定されている場合、設定しても効果はありません。設定すると、このノードの子がすべて削除され、新しい文字列が空または null でない場合は、その文字列を含む単一の Text ノードに置き換えられます。 |
