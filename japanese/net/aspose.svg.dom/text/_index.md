...
description: "Aspose.Svg.Dom.Text クラス。Text インターフェイスは CharacterData から継承され、Element または Attr の XML における文字データと呼ばれるテキストコンテンツを表します。"
type: docs
weight: 3200
url: /ja/net/aspose.svg.dom/text/
---
## Text class

Text インターフェイスは CharacterData を継承し、Element または Attr のテキストコンテンツ（XML では文字データと呼ばれる）を表します。
...
| virtual [Data](../../aspose.svg.dom/characterdata/data/) { get; set; } | このインターフェイスを実装するノードの文字データです。 |
| [FirstChild](../../aspose.svg.dom/node/firstchild/) { get; } | ツリー内でノードの最初の子ノードを返します。子が存在しない場合は null を返します。 |
| [IsElementContentWhitespace](../../aspose.svg.dom/text/iselementcontentwhitespace/) { get; } | このテキストノードが要素コンテンツの空白（しばしば「無視できる空白」と呼ばれる）を含むかどうかを返します。 |
| [LastChild](../../aspose.svg.dom/node/lastchild/) { get; } | ノードの最後の子ノードを返します。親が要素の場合、子は通常要素ノード、テキストノード、またはコメントノードです。子要素がない場合は null を返します。 |
| [Length](../../aspose.svg.dom/characterdata/length/) { get; } | data および下記の substringData メソッドで利用可能な 16 ビット単位の数です。値がゼロになることもあり、つまり CharacterData ノードは空である可能性があります。 |
...
| override [NodeName](../../aspose.svg.dom/text/nodename/) { get; } | このノードの名前（タイプに応じて）。 |
| override [NodeType](../../aspose.svg.dom/text/nodetype/) { get; } | 基になるオブジェクトのタイプを表すコード。 |
| override [NodeValue](../../aspose.svg.dom/text/nodevalue/) { get; set; } | このノードの値（タイプに応じて）。 |
...
| override [TextContent](../../aspose.svg.dom/text/textcontent/) { get; set; } | この属性はこのノードとその子孫のテキスト内容を返します。null に設定されている場合、設定しても効果はありません。設定すると、このノードの子がすべて削除され、新しい文字列が空または null でない場合は、その文字列を含む単一の Text ノードに置き換えられます。 |
| [WholeText](../../aspose.svg.dom/text/wholetext/) { get; } | このノードに論理的に隣接する Text ノードのすべてのテキストを、文書順に連結して返します。 |
...
| virtual [AppendData](../../aspose.svg.dom/characterdata/appenddata/)(*string*) | 文字データの末尾に文字列を追加します。 |
...
| virtual [ReplaceData](../../aspose.svg.dom/characterdata/replacedata/)(*int, int, string*) | 指定された 16 ビット単位オフセットから始まる文字を、指定された文字列に置き換えます。 |
| [ReplaceWholeText](../../aspose.svg.dom/text/replacewholetext/)(*string*) | 現在のノードと論理的に隣接するすべてのテキストノードのテキストを、指定されたテキストに置き換えます。置換テキストの受け取り側でない限り、現在のノードを含むすべての論理的に隣接するテキストノードは削除されます。 |
| [SplitText](../../aspose.svg.dom/text/splittext/)(*int*) | 指定されたオフセットでこのノードを2つに分割し、両方をツリー内の兄弟ノードとして保持します。 |
| virtual [SubstringData](../../aspose.svg.dom/characterdata/substringdata/)(*int, int*) | ノードからデータの範囲を抽出します。 |
| override [ToString](../../aspose.svg.dom/characterdata/tostring/)() | このインスタンスを表す String を返します。 |
