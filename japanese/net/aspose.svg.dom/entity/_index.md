...
description: "Aspose.Svg.Dom.Entity クラス。XML ドキュメント内の既知のエンティティ（解析済みまたは未解析）を表します。"
type: docs
weight: 2850
url: /ja/net/aspose.svg.dom/entity/
---
## Entity class

XML ドキュメント内の、解析済みまたは未解析の既知エンティティを表します。
...
| [InputEncoding](../../aspose.svg.dom/entity/inputencoding/) { get; } | 外部解析済みエンティティの場合、解析時に使用されたエンコーディングを指定する属性です。内部サブセットからのエンティティ、または不明な場合は null になります。 |
...
| override [NodeName](../../aspose.svg.dom/entity/nodename/) { get; } | このノードの名前（タイプに応じて）。 |
| override [NodeType](../../aspose.svg.dom/entity/nodetype/) { get; } | 基になるオブジェクトのタイプを表すコード。 |
| virtual [NodeValue](../../aspose.svg.dom/node/nodevalue/) { get; set; } | 現在のノードの値を取得または設定します。 |
| [NotationName](../../aspose.svg.dom/entity/notationname/) { get; } | 未解析エンティティの場合、エンティティの表記名です。解析済みエンティティの場合は null です。 |
...
| [PublicId](../../aspose.svg.dom/entity/publicid/) { get; } | 指定されている場合はエンティティに関連付けられた公開識別子を返し、そうでない場合は null を返します。 |
| [SystemId](../../aspose.svg.dom/entity/systemid/) { get; } | 指定されている場合はエンティティに関連付けられたシステム識別子を返し、そうでない場合は null を返します。これは絶対 URI の場合もありますし、そうでない場合もあります。 |
| virtual [TextContent](../../aspose.svg.dom/node/textcontent/) { get; set; } | ノードおよびその子孫のテキストコンテンツを表します。 |
| [XmlEncoding](../../aspose.svg.dom/entity/xmlencoding/) { get; } | 外部解析済みエンティティの場合、テキスト宣言の一部としてこのエンティティのエンコーディングを指定する属性です。そうでない場合は null になります。 |
| [XmlVersion](../../aspose.svg.dom/entity/xmlversion/) { get; } | 外部解析済みエンティティの場合、テキスト宣言の一部としてこのエンティティのバージョン番号を指定する属性です。そうでない場合は null になります。 |
