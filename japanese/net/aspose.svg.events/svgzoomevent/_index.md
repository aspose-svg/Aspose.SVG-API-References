...
description: Aspose.Svg.Events.SVGZoomEvent クラス. ズーム イベントはSVG ドキュメント フラグメントの現在のビューを再スケーリングするアクションをユーザーが開始したときに発生しますイベント ハンドラーはsvg 要素でのみ認識されます
type: docs
weight: 1620
...
| [NewScale](../../aspose.svg.events/svgzoomevent/newscale/) { get; } | ズーム操作が処理された後に適用される倍率。 |
| [NewTranslate](../../aspose.svg.events/svgzoomevent/newtranslate/) { get; } | ズーム操作が処理された後に配置される変換値. SVGPoint オブジェクトは読み取り専用です. |
| [PreviousScale](../../aspose.svg.events/svgzoomevent/previousscale/) { get; } | ズーム操作が発生する前に設定されていた、以前のズーム操作からの倍率。 |
| [PreviousTranslate](../../aspose.svg.events/svgzoomevent/previoustranslate/) { get; } | ズーム操作が発生する前に設定されていた、以前のズーム操作からの変換値。 SVGPoint オブジェクトは読み取り専用です。 |
...
| [ZoomRectScreen](../../aspose.svg.events/svgzoomevent/zoomrectscreen/) { get; } | 画面単位で指定されたズーム四角形。 SVGRect オブジェクトは読み取り専用です。 |
