...
description: Aspose.Svg.SVGViewElement クラス. SVGViewElement インターフェイスはview要素に対応します
type: docs
weight: 3590
...
| [ViewTarget](../../aspose.svg/svgviewelement/viewtarget/) { get; } | 指定された「view」要素の属性「viewTarget」に対応します。 「viewTarget」属性にリストされた名前を含む DOMString 値のリスト。 getElementById() メソッド call. を使用して、各 DOMString 値を対応する要素に関連付けることができます。 |
| [ZoomAndPan](../../aspose.svg/svgviewelement/zoomandpan/) { get; set; } | 指定された要素の属性「zoomAndPan」に対応します。値は、このインターフェイスで定義された SVG_ZOOMANDPAN_* 定数のいずれかでなければなりません. |
