...
description: Aspose.Svg.SVGTextPathElement クラス. SVGTextPathElement インターフェイスはtextPath要素に対応します
type: docs
weight: 3540
...
| [LengthAdjust](../../aspose.svg/svgtextcontentelement/lengthadjust/) { get; } | 指定された要素の属性「lengthAdjust」に対応します。値は、このインターフェイスで定義された長さ調整定数のいずれかでなければなりません. |
...
| [GetComputedTextLength](../../aspose.svg/svgtextcontentelement/getcomputedtextlength/)() | この要素内のすべての文字をレンダリングすることによるすべてのアドバンス値の合計。これにはグリフ (水平または垂直) のアドバンス値、プロパティ「kerning」、「letter-spacing」、および「word-」の効果が含まれます。 'tspan' 要素の属性 'dx' および 'dy' による調整。非レンダリング環境の場合、ユーザー エージェントは、グリフ メトリックについて合理的な仮定を行うものとします。 |
...
| [GetNumberOfChars](../../aspose.svg/svgtextcontentelement/getnumberofchars/)() | 現在の要素内でレンダリングできる文字の総数を返します。これには、レンダリングされるかどうかに関係なく、「tref」参照から参照される文字が含まれます。事実上、これは DOM レベル 3 コア ([DOM3]、セクション 1.4) の Node::textContent 属性の長さと同じです (その属性が 'tref' 要素も展開されている場合)。 |
...
| const [TEXTPATH_METHODTYPE_STRETCH](../../aspose.svg/svgtextpathelement/textpath_methodtype_stretch/) | 値「ストレッチ」に対応します。 |
| const [TEXTPATH_METHODTYPE_UNKNOWN](../../aspose.svg/svgtextpathelement/textpath_methodtype_unknown/) | 列挙型が事前定義されたタイプではない値に設定されました。この型の新しい値を定義しようとしたり、既存の値をこの型に切り替えようとしたりすることは無効です. |
...
| const [TEXTPATH_SPACINGTYPE_UNKNOWN](../../aspose.svg/svgtextpathelement/textpath_spacingtype_unknown/) | 列挙型が事前定義されたタイプではない値に設定されました。この型の新しい値を定義しようとしたり、既存の値をこの型に切り替えようとしたりすることは無効です. |
