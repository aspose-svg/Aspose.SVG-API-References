...
description: "SVGBuilderExtensions AddFeConvolveMatrix-metoden. Lägger till en feConvolveMatrix-elementkonfiguration till byggaren. Detta element tillämpar en matriskonvolutionsfiltereffekt"
type: docs
weight: 170
url: /sv/net/aspose.svg.builder/svgbuilderextensions/addfeconvolvematrix/
---
## AddFeConvolveMatrix<TBuilder>(*this TBuilder, Action&lt;SVGFEConvolveMatrixElementBuilder&gt;*) {#addfeconvolvematrix_1}

Lägger till en 'feConvolveMatrix'-elementkonfiguration till byggaren. Detta element applicerar en matriskonvolutionsfiltereffekt.
...
| konfigurera | Konfigurationsåtgärden för elementet 'feConvolveMatrix'. |
...
Lägger till ett 'feConvolveMatrix'-element till SVG-byggaren, som applicerar en matriskonvolutionsfiltereffekt.
...
| byggare | SVG-byggarinstansen som elementet 'feConvolveMatrix' kommer att läggas till. |
| kernelMatrix | Matrisen med värden för konvolutionen. Valfri parameter. |
| divisor | Divisorn för konvolutionen. Valfri parameter. |
| bias | Bias som ska läggas till resultatet av konvolutionen. Valfri parameter. |
| targetX | X-koordinaten för målpixeln i kernel-matrisen. Valfri parameter. |
| targetY | Y-koordinaten för målpixeln i kernel-matrisen. Valfri parameter. |
| edgeMode | Definierar hur kantpixlar hanteras i konvolutionen. Valfri parameter. |
| preserveAlpha | Anger om alfakanalen ska bevaras. Valfri parameter. |
| order | Ordningen på kernel-matrisen. Kan vara ett heltal eller ett ValueTuple av två heltal. Valfri parameter. |
| in | Inmatningen för konvolutionseffekten. Kan vara en sträng eller ett FilterInput. Valfri parameter. |
...
| extend | En valfri åtgärd för att ytterligare konfigurera SVGFEConvolveMatrixElementBuilder. |
