---
title: Class SVGPathSegCurvetoQuadraticSmoothRel
second_title: Referencia de API de Aspose.SVG para .NET
description: Aspose.Svg.Paths.SVGPathSegCurvetoQuadraticSmoothRel clase. La interfaz SVGPathSegCurvetoQuadraticSmoothRel corresponde a un comando de datos de ruta relative smooth cubic curveto t.
type: docs
weight: 2600
url: /es/net/aspose.svg.paths/svgpathsegcurvetoquadraticsmoothrel/
---
## SVGPathSegCurvetoQuadraticSmoothRel class

La interfaz SVGPathSegCurvetoQuadraticSmoothRel corresponde a un comando de datos de ruta "relative smooth cubic curveto" (t).

```csharp
public class SVGPathSegCurvetoQuadraticSmoothRel : SVGPathSeg
```

## Propiedades

| Nombre | Descripción |
| --- | --- |
| [PathSegType](../../aspose.svg.paths/svgpathseg/pathsegtype/) { get; } | El tipo de segmento de ruta especificado por una de las constantes definidas en esta interfaz. |
| [PathSegTypeAsLetter](../../aspose.svg.paths/svgpathseg/pathsegtypeasletter/) { get; } | El tipo del segmento de ruta, especificado por el nombre de comando de un carácter correspondiente. |
| [X](../../aspose.svg.paths/svgpathsegcurvetoquadraticsmoothrel/x/) { get; set; } | La coordenada X relativa para el punto final de este segmento de ruta. |
| [Y](../../aspose.svg.paths/svgpathsegcurvetoquadraticsmoothrel/y/) { get; set; } | La coordenada Y relativa para el punto final de este segmento de ruta. |

## Métodos

| Nombre | Descripción |
| --- | --- |
| [Dispose](../../aspose.svg.datatypes/svgvaluetype/dispose/)() | Libera recursos no administrados y, opcionalmente, administrados. |
| virtual [GetPlatformType](../../aspose.svg.dom/domobject/getplatformtype/)() | Este método se utiliza para recuperar el objeto ECMAScriptType . |

### Ver también

* class [SVGPathSeg](../svgpathseg/)
* espacio de nombres [Aspose.Svg.Paths](../../aspose.svg.paths/)
* asamblea [Aspose.SVG](../../)


