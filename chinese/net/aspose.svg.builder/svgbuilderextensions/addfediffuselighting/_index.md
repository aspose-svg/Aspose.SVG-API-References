...
description: "SVGBuilderExtensions AddFeDiffuseLighting 方法。向构建器添加 feDiffuseLighting 元素的配置。此元素为图像提供光照效果"
type: docs
weight: 180
url: /zh/net/aspose.svg.builder/svgbuilderextensions/addfediffuselighting/
---
## AddFeDiffuseLighting<TBuilder>(*this TBuilder, Action&lt;SVGFEDiffuseLightingElementBuilder&gt;*) {#addfediffuselighting}

向构建器添加一个 'feDiffuseLighting' 元素配置。该元素为图像提供照明效果。
...
| 构建器 | 构建器实例。 |
| 配置 | 针对 'feDiffuseLighting' 元素的配置操作。 |
...
向 SVG 构建器添加一个 'feDiffuseLighting' 元素，使用指定光源应用漫射照明效果。
...
| 构建器 | 将添加 'feDiffuseLighting' 元素的 SVG 构建器实例。 |
| lightSource | 用于配置漫反射光照效果光源的操作。 |
| lightingColor | 光的颜色。可选参数。 |
| surfaceScale | 用于照明效果的表面比例因子。可选参数。 |
| diffuseConstant | 用于确定光照效果的常量。可选参数。 |
| in | 漫反射光照效果的输入。可以是字符串或 FilterInput。可选参数。 |
...
| 扩展 | 一个可选操作，用于进一步配置 SVGFEDiffuseLightingElementBuilder。 |
