...
description: "SVGBuilderExtensions AddFeOffset 方法。向构建器添加 feOffset 元素配置。此元素通过指定的向量偏移输入图像"
type: docs
weight: 260
url: /zh/net/aspose.svg.builder/svgbuilderextensions/addfeoffset/
---
## AddFeOffset<TBuilder>(*this TBuilder, Action&lt;SVGFEOffsetElementBuilder&gt;*) {#addfeoffset}

向构建器添加一个 'feOffset' 元素配置。该元素按指定向量偏移输入图像。
...
向 SVG 构建器添加一个 'feOffset' 元素，通过按指定向量平移输入图像来创建偏移效果。
...
| 构建器 | 将向其添加 ‘feOffset’ 元素的 SVG 构建器实例。 |
| dx | 水平偏移距离。可选参数。 |
| dy | 垂直偏移距离。可选参数。 |
| in | 将应用偏移的输入图像。可以是字符串或 FilterInput。可选参数。 |
...
| 扩展 | 用于进一步配置 SVGFEOffsetElementBuilder 的可选操作。 |
