...
description: "SVGBuilderExtensions AddFeImage 方法。向构建器添加 feImage 元素配置。此元素获取外部图像并将其包含在滤镜管道中"
type: docs
weight: 230
url: /zh/net/aspose.svg.builder/svgbuilderextensions/addfeimage/
---
## AddFeImage<TBuilder>(*this TBuilder, Action&lt;SVGFEImageElementBuilder&gt;*) {#addfeimage}

向构建器添加一个 'feImage' 元素配置。该元素获取外部图像并将其包含在滤镜管道中。
...
向 SVG 构建器添加一个 'feImage' 元素，将外部图像合并到滤镜效果中。
...
| 构建器 | 将向其添加 'feImage' 元素的 SVG 构建器实例。 |
...
| 扩展 | 可选操作，用于进一步配置 SVGFEImageElementBuilder。 |
