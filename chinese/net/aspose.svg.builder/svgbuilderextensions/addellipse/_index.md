...
description: "SVGBuilderExtensions AddEllipse 方法。向构建器添加椭圆元素配置"
type: docs
weight: 120
url: /zh/net/aspose.svg.builder/svgbuilderextensions/addellipse/
---
## AddEllipse<TBuilder>(*this TBuilder, Action&lt;SVGEllipseElementBuilder&gt;*) {#addellipse_1}

向构建器添加一个 'ellipse' 元素配置。
...
向 SVG 构建器添加一个 'ellipse' 元素，指定其中心、半径和样式。
...
| 构建器 | 将向其添加 'ellipse' 元素的 SVG 构建器实例。 |
| cx | 椭圆中心的 x 坐标。可以是 double 值或 double 与 LengthType 的元组。 |
| cy | 椭圆中心的 y 坐标。可以是 double 值或 double 与 LengthType 的元组。 |
| rx | 椭圆的 x 半径。可以是 double 值或 double 与 LengthType 的元组。 |
| ry | 椭圆的 y 半径。可以是 double 值或 double 与 LengthType 的元组。 |
| fill | 椭圆的填充颜色或绘画样式。可以是 Color、Paint 枚举值或绘画服务器 ID。可选参数。 |
| stroke | 椭圆的描边颜色或绘画样式。可以是 Color、Paint 枚举值或绘画服务器 ID。可选参数。 |
| id | 椭圆元素的唯一标识符。可选参数。 |
| 扩展 | 用于进一步配置椭圆元素构建器的可选操作。 |
