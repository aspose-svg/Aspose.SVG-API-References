...
description: Aspose.Svg.Dom.Css.ICSSRule 界面. CSSRule 接口是任何类型的CSS 语句的抽象基接口这包括规则集和规则期望实现保留 CSS 样式表中指定的所有规则即使解析器无法识别该规则无法识别的规则使用ICSSUnknownRule接口.
type: docs
weight: 620
...
| [Type](../../aspose.svg.dom.css/icssrule/type/) { get; } | 规则的类型，如上定义。预期是特定于绑定的转换方法可用于从 CSSRule 接口的实例向下转换为类型隐含的特定派生接口。 |
