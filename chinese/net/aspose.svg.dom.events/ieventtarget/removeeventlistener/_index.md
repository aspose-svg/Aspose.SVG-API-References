...
此方法允许从事件目标中删除事件侦听器。 如果[`IEventListener`](../../ieventlistener)被从一个[`EventTarget`](../../../aspose.svg.dom/eventtarget)在处理事件时，不会被当前操作触发。 事件监听器被移除后永远无法调用。
...
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| type | String | 指定事件类型[`IEventListener`](../../ieventlistener)被移除。 |
