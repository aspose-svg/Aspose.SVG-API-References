...
description: 的Event./event用于向处理事件的处理程序提供有关事件的上下文信息
type: docs
weight: 920
...
的[`Event`](../event)用于向处理事件的处理程序提供有关事件的上下文信息。
...
| 姓名 | 描述 |
| --- | --- |
| [Event](event#constructor)(string) | 初始化[`Event`](../event)类. |
| [Event](event#constructor_1)(string, IDictionary&lt;string, object&gt;) | 初始化[`Event`](../event)类. |
...
| [InitEvent](../../aspose.svg.dom.events/event/initevent)(string, bool, bool) | 的[`InitEvent`](./initevent)方法用于初始化一个值[`Event`](../event)通过 创建[`IDocumentEvent`](../idocumentevent)接口. |
| [PreventDefault](../../aspose.svg.dom.events/event/preventdefault)() | 如果事件是可取消的，则[`PreventDefault`](./preventdefault)方法用于表示事件将被取消， 表示实现通常作为事件结果而采取的任何默认操作都不会发生。 |
...
| const [CapturingPhase](../../aspose.svg.dom.events/event/capturingphase) | 目标当前正在评估事件[`IEventTarget`](../ieventtarget) . |
| const [NonePhase](../../aspose.svg.dom.events/event/nonephase) | 当前未调度的事件处于此阶段。 |

### 评论

一个实现[`Event`](../event)通常作为第一个参数传递给事件处理程序。 更具体的上下文信息通过派生附加接口传递给事件处理程序[`Event`](../event) 包含与它们伴随的事件类型直接相关的信息。 这些派生接口也由传递给事件侦听器的对象实现。
