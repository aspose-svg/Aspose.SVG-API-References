...
| [Data](../../aspose.svg.dom.events/inputevent/data) { get; } | 数据保存输入法生成的字符的值。这可能是单个 Unicode 字符或非空的 Unicode 字符序列 [Unicode]。字符应该按照 [UAX15] 中定义的 Unicode 规范化形式 NFC 的定义进行规范化。该属性可以包含空字符串。 |
...
| [IsComposing](../../aspose.svg.dom.events/inputevent/iscomposing) { get; } | 如果输入事件作为合成会话的一部分发生，即在合成开始事件之后和对应的合成结束事件之前，则为真。该属性的未初始化值必须为假。 |
