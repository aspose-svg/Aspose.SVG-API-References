...
description: Aspose.Svg.Dom.ProcessingInstruction 班级. ProcessingInstruction 表示处理指令在 XML 中用作在文档文本中保留处理器特定信息的方法
type: docs
weight: 1170
...
ProcessingInstruction 表示“处理指令”，在 XML 中用作在文档文本中保留处理器特定信息的方法。
...
| override [NodeValue](../../aspose.svg.dom/processinginstruction/nodevalue/) { get; set; } | 这个节点的值，取决于它的类型。 |
...
| [Target](../../aspose.svg.dom/processinginstruction/target/) { get; } | 这条处理指令的目标。 |
| override [TextContent](../../aspose.svg.dom/processinginstruction/textcontent/) { get; set; } | 该属性返回该节点及其后代的文本内容。当它被定义为空时，设置它是无效的。在设置时，此节点可能具有的任何可能的子节点都将被删除，如果新字符串不为空或为空，则替换为包含此属性设置为的字符串的单个文本节点。 |
