...
description: "Aspose.Svg.Dom.Node 类。Node 接口是整个 Document 对象模型的主要数据类型。它表示文档树中的单个节点。"
type: docs
weight: 3140
url: /zh/net/aspose.svg.dom/node/
---
## Node class

Node 接口是整个文档对象模型的主要数据类型。它表示文档树中的单个节点。
...
| virtual [LocalName](../../aspose.svg.dom/node/localname/) { get; } | 返回此节点的限定名称的本地部分。对于除 [`ELEMENT_NODE`](./element_node/) 和 [`ATTRIBUTE_NODE`](./attribute_node/) 之外的任何类型的节点，以及使用 DOM Level 1 方法（例如 [`CreateElement`](../document/createelement/)）创建的节点，此值始终为 null。 |
| virtual [NamespaceURI](../../aspose.svg.dom/node/namespaceuri/) { get; } | 返回元素的命名空间 URI，如果元素不在命名空间中，则返回 null。 |
| [NextSibling](../../aspose.svg.dom/node/nextsibling/) { get; } | 返回其父节点的 [`ChildNodes`](./childnodes/) 中紧随指定节点之后的节点；如果指定节点是父元素的最后一个子节点，则返回 null。 |
| abstract [NodeName](../../aspose.svg.dom/node/nodename/) { get; } | 以字符串形式返回当前节点的名称。 |
| abstract [NodeType](../../aspose.svg.dom/node/nodetype/) { get; } | 表示底层对象类型的代码。 |
...
| [PreviousSibling](../../aspose.svg.dom/node/previoussibling/) { get; } | 返回其父节点的 [`ChildNodes`](./childnodes/) 列表中紧邻指定节点之前的节点；如果指定节点是该列表中的第一个，则返回 null。 |
...
| [AppendChild](../../aspose.svg.dom/node/appendchild/)(*Node*) | 将节点添加到指定父节点的子节点列表末尾。如果给定的子节点是文档中已有节点的引用，[`AppendChild`](./appendchild/) 会将其从当前位置移动到新位置（在将节点附加到其他节点之前，无需先从其父节点中移除）。 |
| [CloneNode](../../aspose.svg.dom/node/clonenode/#clonenode)() | 返回调用此方法的节点的副本。 |
| [CloneNode](../../aspose.svg.dom/node/clonenode/#clonenode_1)(*bool*) | 返回调用此方法的节点的副本。其参数决定是否同时克隆节点中包含的子树。 |
...
| [HasChildNodes](../../aspose.svg.dom/node/haschildnodes/)() | 返回一个布尔值，指示给定的 `Node` 是否具有子节点。 |
| [InsertBefore](../../aspose.svg.dom/node/insertbefore/)(*Node, Node*) | 在现有子节点 child 之前插入该节点。如果 child 为 null，则将节点插入到子节点列表的末尾。如果 child 是 DocumentFragment 对象，则其所有子节点按相同顺序在 child 之前插入。如果该子节点已经在树中，则会先将其移除。 |
| [IsDefaultNamespace](../../aspose.svg.dom/node/isdefaultnamespace/)(*string*) | 此方法检查指定的 namespaceURI 是否为默认命名空间。 |
| [IsEqualNode](../../aspose.svg.dom/node/isequalnode/)(*Node*) | 测试两个节点是否相等。此方法测试节点的相等性，而非同一性（即两个节点是否引用同一对象），后者可通过 Node.isSameNode() 进行测试。所有相同的节点也会相等，但反之未必成立。 |
| [IsSameNode](../../aspose.svg.dom/node/issamenode/)(*Node*) | 该方法是 === 严格相等运算符的旧别名。即，它测试两个节点是否相同（换句话说，它们是否引用同一对象）。 |
...
| [RemoveChild](../../aspose.svg.dom/node/removechild/)(*Node*) | 从 DOM 中移除一个子节点并返回被移除的节点。 |
...
| [ReplaceChild](../../aspose.svg.dom/node/replacechild/)(*Node, Node*) | 在子节点列表中用 newChild 替换子节点 oldChild，并返回 oldChild 节点。如果 newChild 是 DocumentFragment 对象，oldChild 将被 DocumentFragment 的所有子节点替换，这些子节点按相同顺序插入。如果 newChild 已经在树中，则会先将其移除。 |
...
| 名称 | 描述 |
| --- | --- |
| const [ATTRIBUTE_NODE](../../aspose.svg.dom/node/attribute_node/) | 属性节点 |
| const [CDATA_SECTION_NODE](../../aspose.svg.dom/node/cdata_section_node/) | 一个 CDATA 区段节点 |
| const [COMMENT_NODE](../../aspose.svg.dom/node/comment_node/) | 一个注释节点 |
| const [DOCUMENT_FRAGMENT_NODE](../../aspose.svg.dom/node/document_fragment_node/) | 一个文档片段节点 |
| const [DOCUMENT_NODE](../../aspose.svg.dom/node/document_node/) | 一个文档节点 |
| const [DOCUMENT_TYPE_NODE](../../aspose.svg.dom/node/document_type_node/) | 一个文档类型节点 |
| const [ELEMENT_NODE](../../aspose.svg.dom/node/element_node/) | 一个元素节点 |
| const [ENTITY_NODE](../../aspose.svg.dom/node/entity_node/) | 一个实体节点 |
| const [ENTITY_REFERENCE_NODE](../../aspose.svg.dom/node/entity_reference_node/) | 一个实体引用节点 |
| const [NOTATION_NODE](../../aspose.svg.dom/node/notation_node/) | 一个标记节点 |
| const [PROCESSING_INSTRUCTION_NODE](../../aspose.svg.dom/node/processing_instruction_node/) | 一个处理指令节点 |
| const [TEXT_NODE](../../aspose.svg.dom/node/text_node/) | 一个文本节点 |
