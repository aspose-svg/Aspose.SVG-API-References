...
description: Aspose.Svg.Dom.Text 班级. Text 接口继承自 CharacterData 并表示元素或属性的文本内容在 XML 中称为字符数据
type: docs
weight: 1200
...
| [IsElementContentWhitespace](../../aspose.svg.dom/text/iselementcontentwhitespace/) { get; } | 返回此文本节点是否包含元素内容空白，通常被滥用为“可忽略的空白”。 |
| [LastChild](../../aspose.svg.dom/node/lastchild/) { get; } | 此节点的最后一个子节点。如果没有这样的节点，则返回 null. |
| [Length](../../aspose.svg.dom/characterdata/length/) { get; } | 可通过数据和下面的 substringData 方法获得的 16 位单元的数量。这可能具有零值，即 CharacterData 节点可能为空。 |
...
| override [NodeValue](../../aspose.svg.dom/text/nodevalue/) { get; set; } | 这个节点的值，取决于它的类型。 |
...
| override [TextContent](../../aspose.svg.dom/text/textcontent/) { get; set; } | 该属性返回该节点及其后代的文本内容。当它被定义为空时，设置它是无效的。在设置时，此节点可能具有的任何可能的子节点都将被删除，如果新字符串不为空或为空，则替换为包含此属性设置为的字符串的单个文本节点。 |
| [WholeText](../../aspose.svg.dom/text/wholetext/) { get; } | 返回逻辑上与该节点相邻的文本节点的文本节点的所有文本，按文档顺序连接。 |
...
| virtual [AppendData](../../aspose.svg.dom/characterdata/appenddata/)(string) | 将字符串附加到节点的字符数据的末尾。 |
...
| virtual [ReplaceData](../../aspose.svg.dom/characterdata/replacedata/)(int, int, string) | 用指定的字符串替换从指定的 16 位单元偏移量开始的字符。 |
| [ReplaceWholeText](../../aspose.svg.dom/text/replacewholetext/)(string) | 用指定文本替换当前节点和所有逻辑上相邻的文本节点的文本。删除所有逻辑上相邻的文本节点，包括当前节点，除非它是替换文本的接收者。 |
| [SplitText](../../aspose.svg.dom/text/splittext/)(int) | 将此节点分成指定偏移处的两个节点，将它们作为兄弟节点保留在树中。 |
| virtual [SubstringData](../../aspose.svg.dom/characterdata/substringdata/)(int, int) | 从节点中提取一系列数据。 |
| override [ToString](../../aspose.svg.dom/characterdata/tostring/)() | 返回一个String代表这个实例. |
