---
title: "Color.FromOklcha"
second_title: "Aspose.SVG for .NET API 参考"
description: "Color FromOklcha 方法。返回 OKLAB 模型中具有请求的亮度、色度、色相和 alpha 值的新颜色。"
type: docs
weight: 190
url: /zh/net/aspose.svg.drawing/color/fromoklcha/
---
## Color.FromOklcha method

返回一个针对 OKLAB 模型、具有请求的亮度、色度、色相和 alpha 值的新 Color。

```csharp
public static Color FromOklcha(float luminance, float chroma, float hue, float alpha)
```

| 参数 | 类型 | 描述 |
| --- | --- | --- |
| 亮度 | Single | 一个浮点数，表示颜色的亮度分量。 |
| 色度 | Single | 一个浮点数，表示颜色的色度分量。 |
| 色相 | Single | 一个浮点数，表示颜色的色相分量。 |
| 透明度 | Single | 一个浮点数，表示颜色的透明度分量。 |

### 返回值

一个新的 [`Color`](../) 类实例

### 另请参阅

* class [Color](../)
* namespace [Aspose.Svg.Drawing](../../../aspose.svg.drawing/)
* assembly [Aspose.SVG](../../../)
