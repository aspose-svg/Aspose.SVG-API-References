---
title: Color
second_title: Aspose.SVG for .NET API 参考
description: 初始化Coloraspose.svg.drawing/color class. 默认颜色为黑色
type: docs
weight: 10
url: /zh/net/aspose.svg.drawing/color/color/
---
## Color() {#constructor}

初始化[`Color`](../../color) class. 默认颜色为黑色。

```csharp
public Color()
```

### 也可以看看

* class [Color](../../color)
* 命名空间 [Aspose.Svg.Drawing](../../color)
* 部件 [Aspose.SVG](../../../)

---

## Color(float, float, float) {#constructor_5}

初始化[`Color`](../../color) class. 所有颜色分量必须在 0-1 范围内。

```csharp
public Color(float red, float green, float blue)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| red | Single | 颜色的红色分量。 |
| green | Single | 颜色的绿色分量。 |
| blue | Single | 颜色的蓝色分量。 |

### 也可以看看

* class [Color](../../color)
* 命名空间 [Aspose.Svg.Drawing](../../color)
* 部件 [Aspose.SVG](../../../)

---

## Color(float, float, float, float) {#constructor_6}

初始化[`Color`](../../color) class. 所有颜色分量必须在 0-1 范围内。

```csharp
public Color(float red, float green, float blue, float alpha)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| red | Single | 颜色的红色分量。 |
| green | Single | 颜色的绿色分量。 |
| blue | Single | 颜色的蓝色分量。 |
| alpha | Single | 颜色的 alpha 分量。 |

### 也可以看看

* class [Color](../../color)
* 命名空间 [Aspose.Svg.Drawing](../../color)
* 部件 [Aspose.SVG](../../../)

---

## Color(byte, byte, byte) {#constructor_1}

初始化[`Color`](../../color)class. 所有颜色分量必须在 0-255. 范围内

```csharp
public Color(byte red, byte green, byte blue)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| red | Byte | 颜色的红色分量。 |
| green | Byte | 颜色的绿色分量。 |
| blue | Byte | 颜色的蓝色分量。 |

### 也可以看看

* class [Color](../../color)
* 命名空间 [Aspose.Svg.Drawing](../../color)
* 部件 [Aspose.SVG](../../../)

---

## Color(byte, byte, byte, byte) {#constructor_2}

初始化[`Color`](../../color)class. 所有颜色分量必须在 0-255. 范围内

```csharp
public Color(byte red, byte green, byte blue, byte alpha)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| red | Byte | 表示颜色的红色分量的字节。 |
| green | Byte | 表示颜色的绿色分量的字节。 |
| blue | Byte | 表示颜色的蓝色分量的字节。 |
| alpha | Byte | 一个字节，表示颜色的 alpha 分量。 |

### 也可以看看

* class [Color](../../color)
* 命名空间 [Aspose.Svg.Drawing](../../color)
* 部件 [Aspose.SVG](../../../)

---

## Color(int, int, int) {#constructor_3}

初始化[`Color`](../../color)class. 所有颜色分量必须在 0-255. 范围内

```csharp
public Color(int red, int green, int blue)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| red | Int32 | 颜色的红色分量。 |
| green | Int32 | 颜色的绿色分量。 |
| blue | Int32 | 颜色的蓝色分量。 |

### 也可以看看

* class [Color](../../color)
* 命名空间 [Aspose.Svg.Drawing](../../color)
* 部件 [Aspose.SVG](../../../)

---

## Color(int, int, int, int) {#constructor_4}

初始化[`Color`](../../color)class. 所有颜色分量必须在 0-255. 范围内

```csharp
public Color(int red, int green, int blue, int alpha)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| red | Int32 | 颜色的红色分量。 |
| green | Int32 | 颜色的绿色分量。 |
| blue | Int32 | 颜色的蓝色分量。 |
| alpha | Int32 | 颜色的 alpha 分量。 |

### 也可以看看

* class [Color](../../color)
* 命名空间 [Aspose.Svg.Drawing](../../color)
* 部件 [Aspose.SVG](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.SVG.dll -->
